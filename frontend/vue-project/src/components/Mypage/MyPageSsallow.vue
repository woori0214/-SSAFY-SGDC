<template>
  <div>
    ...
    <transition>
      <div
        class="accordion-content-ssallow"
        :class="{ open: isOpen }"
        v-show="isOpen"
      >
        <div class="sallowing_body">
          <h2>쌀로잉</h2>
          <div
            v-for="ssallowing in ssallowings"
            :key="ssallowing.user_id"
            class="ssallow"
          >
            ...
            <div class="ssallow_btns">
              <button
                @click="gotoProfile(ssallowing.user_id)"
                class="ssallow_btn"
              >
                프로필 페이지
              </button>
              <button @click="toggleFollow(ssallowing)" class="ssallow_btn">
                {{ getButtonText(ssallowing.isFollowing) }}
              </button>
            </div>
          </div>
        </div>
        <div class="ssallower_body">
          <h2>쌀로워</h2>
          <div
            v-for="ssallower in ssallowers"
            :key="ssallower.user_id"
            class="ssallow"
          >
            <div class="ssallow_info">
            ...
            <div class="ssallow_btns">
              <button
                @click="gotoProfile(ssallower.user_id)"
                class="ssallow_btn"
              >
                프로필 페이지
              </button>
              <button @click="toggleFollow(ssallower)" class="ssallow_btn">
                {{ getButtonText(ssallower.isFollowing) }}
              </button>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup(props) {
    ...
    const ssallowings = ref(
      props.ssallowingData.map((ssallowing) => ({
        ...ssallowing,
        isFollowing: true,
      }))
    );
    ...
    const ssallowers = ref(
      props.ssallowerData.map((ssallower) => ({
        ...ssallower,
        isFollowing: false,
      }))
    );
    ...
    const goUnssallow = function (followingId) {
      const unssallow_info = {
        user_id: mypageUser.value,
        following_id: followingId,
      };
      follow
        .deleteSsallowing(unssallow_info)
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
    ...
    const goSsallowing = function (followId) {
      const ssallowing_info = {
        user_id: mypageUser.value,
        following_id: followId,
      };
      follow
        .plusSsallowing(ssallowing_info)
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
    ...
    const updateFollowStatus = () => {
      ...
      const ssallowerPromises = ssallowers.value.map((ssallower) => {
        const checkData = {
          user_id: mypageUser.value,
          following_id: ssallower.user_id,
        };
        return follow
          .checkSsallowing(checkData)
          .then((response) => {
